...
if __name__ == "__main__":
    ...
    dominant_colors = get_dominant_colours(currentWallpaper, count=5)
    tint_color = choose_tint_color(dominant_colors, background_color=(0, 0, 0))

    # print(tint_color)
    color= "%02x%02x%02x" % tuple(int(v * 255) for v in tint_color)
    cmd ='ratbagctl warbling-mara led 0 set color '+ color
    os.system(cmd)